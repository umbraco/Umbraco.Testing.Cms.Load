...
**Supported Umbraco versions: v13–v18 (subject to seeder availability).** Each major maps to a specific .NET runtime (see [Umbraco-major → .NET-runtime map](#umbraco-major--net-runtime-map) below). Today **v13, v17, and v18** have a published `Umbraco.Cms.TestDataSeeder` build; v14–v16 queues fail at validation with a clear message until the seeder ships for those majors. The version-per-major map lives in `scripts/seeder-versions.json` — a single file read by both `scripts/resolve-run-config.ps1` (validator-side) and `Terraform/modules/umbraco/scripts/install-umbraco-cms-on-appservice.ps1` (install-side); add an entry there when a new seeder version ships.
...
|-----------|-------------|---------|---------|
| `umbracoVersion` | Umbraco CMS version. Free-text — accepts prereleases (`17.0.0-rc.1`, `17.1.0-beta.2`). The validator accepts v13, v17, v18 today (the majors with a published `Umbraco.Cms.TestDataSeeder` build - see `scripts/seeder-versions.json`); v14/v15/v16 fail validation with a "seeder hasn't shipped" message. Also checked against nuget.org directly - a version string that doesn't resolve there (typo, or a prerelease/nightly-only build) fails validation with a clear message instead of surfacing 10+ minutes into provisioning. The major segment maps to the .NET runtime automatically (see table below). | 17.0.0 | free text |
...
**Selecting which plans run.** The `backofficePlans` queue parameter lists the plans to run; delete entries at queue time to skip them (e.g. drop `SaveDocumentType` when you don't want it tanking a run — see below — or keep only the write plans). It's ignored for the frontend workload, and a plan with no `.jmx` for the resolved major is skipped regardless. All plans share the resolved backoffice VU count — there's no per-plan VU knob (that would sacrifice cross-plan comparability); scope a run by *which* plans it includes, not by per-plan concurrency.

**`SaveDocumentType` is a known outlier, not a v18-specific issue.** Confirmed live on v17.6.1 (Starter/Standard/Pro): its "Update name and save document type" step shows a severe, consistent tail latency (~60-230s p95/p99 depending on tier) and high error rates (30-40%+), tied to real `SqlException: Lock request time out period exceeded` errors server-side. The mechanism: this step PUTs a randomly-selected document type from the seeded pool, and — unlike content operations, which pick from Medium's ~500 seeded documents — the document-type pool is small and fixed regardless of seeder preset (~4-5 types; see the content model above). 15 concurrent backoffice VUs against ~4-5 document types collide often; scaling the App Service tier (tested through Pro) reduces the fast-path latency but doesn't move the tail, which is pinned near a fixed lock/command timeout rather than compute capacity. The load-test's absolute latency gate is relaxed for this one plan (`generate-loadtest-config.ps1`'s `$heavyWriteJmx`) for this reason; drop it from `backofficePlans` if you want a clean run without it.

**State across the loop is intentionally shared.** Iterations run against the same warm App Service without re-seeding between them. SaveContent.jmx creates content; SaveDocumentType.jmx modifies schema; PublishContent.jmx runs against the cumulative state. This mirrors mixed-workload backoffice use, but means later iterations don't measure clean isolated performance. To compare a single `.jmx` cleanly across runs, slice on the `Sampler` dimension — for backoffice that's `scenario_name`, prefixed with the `.jmx` stem (e.g. `SaveContent / 01. …`), which is what the workbook charts group on.
...
- **Unsupported Umbraco major**: "Umbraco.Cms.TestDataSeeder hasn't shipped a build for major N yet…" — add an entry to `scripts/seeder-versions.json`, the single source of truth both `resolve-run-config.ps1` and `Terraform/modules/umbraco/scripts/install-umbraco-cms-on-appservice.ps1` read.
- **Umbraco version not on nuget.org**: "Umbraco.Templates X.Y.Z was not found on nuget.org…" — the requested `umbracoVersion` doesn't resolve on nuget.org's flat-container index (typo, or a version that only exists on a prerelease/nightly feed). Verify the version string before re-queuing.
...
- `requests_per_sec` — throughput per task and aggregate. Denominator is the **observed sample window** (`max(timeStamp) - min(timeStamp)` across the kept rows), not the configured test duration: the two diverge whenever ALT autoStops early or the run fast-fails, and using the configured value understated throughput by exactly the fraction of the window that never ran — worst on the saturated runs where throughput is the number you care about. Falls back to the configured duration when the CSV has no usable timestamps. **`requests_per_sec` on rows published before this change is not comparable for truncated runs**; full-length runs are unaffected (observed span ≈ configured duration).
